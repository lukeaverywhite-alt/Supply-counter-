...
| Normal runtime composition and automatic refresh | Passed locally | Injected dependencies, durable encrypted `EventSyncProvider`, cleanup-safe polling/reconnect bridge, and a two-client controller test with distinct Web Crypto identities and repositories. Existing count-session setup UI remains limited. |
...
The downloadable static client still needs a deployed relay plus an organization enrollment package before two physical devices can share data. The application now has a real, durable encrypted transport adapter for that composition: it persists ciphertext before send, reuses it after an ambiguous acknowledgement, paginates all history, and lets replica replay atomically validate signatures and permissions. It does not silently substitute an in-memory provider when operational dependencies are supplied. A hosted relay and safe enrollment/key-distribution workflow remain external deployment prerequisites rather than fake in-browser services.
