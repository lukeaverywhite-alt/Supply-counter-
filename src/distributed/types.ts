...
export type CurrentPropertyLine = { propertyId: string; itemId: string; label: string; size?: string; quantity: number; issuedAt: string; issueEventId: string; issueTransactionId: string; bundleId?: string; bundleVersion?: number }
export type CadetProjection = { cadetId: string; fullName: string; gender: CadetGender; nsLevel: NsLevel; status: 'ACTIVE' | 'INACTIVE'; sizes: Record<string, string>; currentProperty: CurrentPropertyLine[]; createdAt: string; updatedAt: string; version: number; appliedEventIds: string[] }
...
export type StillNeededProjection = { requirementId: string; cadetId: string; itemId?: string; displayLabel: string; size?: string; quantityNeeded: number; quantityFulfilled: number; status: 'OPEN' | 'PARTIALLY_FULFILLED' | 'FULFILLED' | 'CANCELLED'; firstNeededAt: string; updatedAt: string; source: 'MANUAL' | 'INCOMPLETE_ISSUE' | 'CORRECTION'; relatedTransactionId?: string; relatedBundleId?: string; bundleVersion?: number; version: number; appliedEventIds: string[] }
export type SupplyTransactionLine = { lineId: string; itemId: string; label: string; size?: string; quantity: number; baseVersion: number; propertyId?: string; requirementId?: string }
export type MissingIssueLine = { lineId: string; itemId?: string; label: string; size?: string; quantity: number; required: true }
