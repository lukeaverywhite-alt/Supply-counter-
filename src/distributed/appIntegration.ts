...
import type { BundleProjection, CadetProjection, ConflictRecord, InventoryProjection, StillNeededProjection, StoredEvent } from './types'
...
export class DistributedAppController {
  ...
  async project(): Promise<ArgusAppProjection> { const state = await this.repository.snapshot(); return { inventory: state.inventory, cadets: state.cadets.map(cadet => ({ ...cadet, propertyCount: cadet.currentProperty.reduce((sum, item) => sum + item.quantity, 0), stillNeededCount: state.stillNeeded.filter(item => item.cadetId === cadet.cadetId && ['OPEN','PARTIALLY_FULFILLED'].includes(item.status)).length, readiness: cadetReadiness(state.stillNeeded.filter(item => item.cadetId === cadet.cadetId)) })), bundles: state.bundles.map(bundle => ({ ...bundle, mapping: bundleMapping(bundle, state.inventory) })), stillNeeded: state.stillNeeded.map(requirement => ({ ...requirement, availability: requirementAvailability(requirement, state.inventory) })), conflicts: state.conflicts, events: state.events, audit: auditFrom(state), sync: { outbox: state.outbox.length, openConflicts: state.conflicts.filter(conflict => conflict.status === 'OPEN').length }, integrity: inspectRepository(state) } }
...
export type ArgusAppProjection = { inventory: InventoryProjection[]; cadets: Array<CadetProjection & { propertyCount: number; stillNeededCount: number; readiness: ReturnType<typeof cadetReadiness> }>; bundles: Array<BundleProjection & { mapping: BundleMapping }>; stillNeeded: Array<StillNeededProjection & { availability: ReturnType<typeof requirementAvailability> }>; conflicts: ConflictRecord[]; events: StoredEvent[]; audit: AuditEvent[]; sync: { outbox: number; openConflicts: number }; integrity: IntegrityReport }
