...
describe('IndexedDbRepository migrations', () => {
  ...
  it('rejects asynchronous transaction callbacks before IndexedDB can auto-close', async () => {
    useFreshIndexedDb(); const repository = new IndexedDbRepository('async-callback')
    await repository.initialize()
    const asyncChange: Parameters<IndexedDbRepository['transaction']>[0] = async state => { state.quarantine.push({ eventId:'unsafe', reason:'async callback', receivedAt:'2026-01-01T00:00:00Z' }); await Promise.resolve() }
    await expect(repository.transaction(asyncChange)).rejects.toThrow(/must be synchronous/)
    expect((await repository.snapshot()).quarantine).toEqual([])
    repository.close()
  })
